export default function OpenModalImage() {
  const openModalImage = ({ target }: Event) => {
    const elementModalImage = document.querySelector<HTMLElement>(
      ".modal-image-mobile",
    );
    ...
    if (isMobile) {
      if (elementBody && target && target instanceof Element) {
        if (
          target.nextSibling && target.nextSibling instanceof HTMLImageElement
        ) {
          const src = target.nextSibling.getAttribute("src");
          if (src && elementModalImage) {
            // @ts-ignore: Ignorando erro
            elementModalImage.querySelector<HTMLImageElement>("img")
  ...
  return (
    <>
      <div
        class="overlay-image absolute w-full h-full left-0"
