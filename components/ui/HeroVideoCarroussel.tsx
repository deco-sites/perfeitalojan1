import type { Props } from "../ui/HeroSection.tsx";
import Button from "../ui/Button.tsx";
import Icon from "../ui/Icon.tsx";
import { useSignal } from "@preact/signals";
import { useState } from "preact/hooks";
import HeroSectionComponent from "../ui/HeroSection.tsx";
...
export default function HeroVideoCarroussel(
  { banner, slider, isHeaderTransparent }: BannerProps,
) {
  ...
  const nextBanner = () => {
    ...
    const element = document.querySelector<HTMLElement>(
      ".custom-banner-slide.is-init",
    );
    const elementLoading = document.querySelector<HTMLElement>(
      ".loading-container",
    );
    if (element) {
  ...
  return (
    <div
      class={`custom-banner-one relative ${
        isHeaderTransparent ? "" : "lg:mt-[99px] mt-[60px]"
      }`}
    >
      <div class="custom-banner-one__container flex flex-row overflow-auto scrollbar-none">
        ...
        {banner.map((ban, index) => (
          <div
            key={index}
            class={`custom-banner-slide children:w-screen -ml-[calc(100vw*${index})] ${
              index === inFocus.value
      ...
      {banner.length > 1 && (
        <>
          <div
            class={`custom-banner-slide flex absolute w-full bottom-[15px] lg:bottom-[70px] z-10 gap-2 justify-center`}
          >
